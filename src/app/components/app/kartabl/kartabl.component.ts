...
export class KartablComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.model.tasks.forEach(t => {
      const project = this.projectService.projects.find(p => p.id === t.projectId);
  ...
  switchProject(selected: any) {
    ...
    const filteredTasks = this.model.tasks.filter(t => t.packageId === selected);
    this.project = payload.project;
    this.pkg = {...payload.pkg, lists: []};
    this.pkg.lists = filteredTasks.filter(f => {
