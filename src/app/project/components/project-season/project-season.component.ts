...
export class ProjectSeasonComponent implements OnInit {
  ...
  async prepareDelete(season: ProjectSeasonViewModel) {
    ...
    const response = await this.modalService
      .confirm({
        title: 'REMOVE_SEASON',
        subTitle: 'REMOVE_SEASON_CONFIRM',
        description: heading,
        confirmLabel: 'REMOVE_SEASON',
        cancelLabel: 'CANCEL'
      });

    if (response.confirmed) {
      await this.projectService.removeSeason(season.id);
    }
  }
